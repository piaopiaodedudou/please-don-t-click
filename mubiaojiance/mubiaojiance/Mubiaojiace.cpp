...
*车辆检测主体程序
********************************************************/
void detectcars(Mat& img,CascadeClassifier& cascade,vector<Rect>& cars)
{
	Mat Img(cvRound(img.rows/scale),cvRound(img.cols/scale),CV_8UC3);
	resize(img,Img,Img.size(),0,0,INTER_LINEAR);

	Mat ImgROI;
	double H = 0.4*Img.rows;
	ImgROI = Img(Range(H,Img.rows),Range(0,Img.cols));

	cascade.detectMultiScale(ImgROI,cars,multiscale,3,0,Size(50,50),Size(700,700));

	for(vector<Rect>::iterator iter = cars.begin();iter != cars.end();++ iter)
	{
		iter->x = iter->x * scale;
		iter->y = (iter->y + H) * scale;
		iter->width = iter->width * scale;
		iter->height = iter->height * scale;
	}
	rangeRect(cars);                                                             //对检测目标进行排序
    CombineRect(cars);                                                           //合并重复框
    //FindRect(cars,scale,Imgwidth,Imgheight);                                   //扩大检测框
...
void CombineRect(vector<Rect>& rect)
{
	if(rect.size() > 1)
	{
		...
		{
			if(calOverlapRatio(*iter,*(iter + 1)) > 0.4)
			{
				//(iter + 1)->x = (iter->x + (iter + 1)->x)*0.5;
				//(iter + 1)->y = (iter->y + (iter + 1)->y)*0.5;
				//(iter + 1)->width = (iter->width + (iter + 1)->width)*0.5;
				//(iter + 1)->height = (iter->height + (iter + 1)->height)*0.5;
				(iter + 1)->x = iter->x*0.8 + (iter + 1)->x*0.2;                   //对重复框：小*0.8 + 大*0.2
				(iter + 1)->y = iter->y*0.8 + (iter + 1)->y*0.2;                                     
				(iter + 1)->width = iter->width*0.8 + (iter + 1)->width*0.2;        
				(iter + 1)->height = iter->height*0.8 + (iter + 1)->height*0.2;
...
*对无重复的检测框进行扩大，用于消除抖动
*********************************************************/
vector<Rect>& FindRect(vector<Rect> &rect,double scale,int w,int h)
{
	
	if(rect.size() > 0)
	{
		int movePix = ceil((sampleszie*(pow(scale,1) - 1))/2);
		for(vector<Rect>::iterator iter = rect.begin();iter != rect.end(); ++iter )
		{
			iter->x = (iter->x > movePix) ? iter->x - movePix:0;
			iter->y = (iter->y > movePix) ? iter->y - movePix:0;
			iter->width = ((iter->x + iter->width + 2*movePix) < w) ? iter->width + 2*movePix:w - iter->x;
			iter->height = ((iter->y + iter->width + 2*movePix) < h) ? iter->height + 2*movePix:h - iter->y;
		}
		return rect;
	}
	else
...
double calOverlapRatio(Rect rect1,Rect rect2)
{
	...
	double ratio = static_cast<double>(tempRect.area())/max(rect1.area(),rect2.area());
	return ratio;
}
/********************************************************
*判定新帧的检测框是否属于抖动(参数待定)
...
//	{
//		xielv = MatchRectMove(HisRect,newrect);
//		if(xielv.size() == 0)
//		{
//			result.push_back(1);
//		}
//		else
//		{
//			for(vector<MoveMessage>::const_iterator it = xielv.begin();it != xielv.end(); ++it)
//			{
//				if(it->x < 80 || (it->x + it->width) > (Imgwidth - 80) )  //判断条件有问题，这个应该是移除视野的情况
//				{
//					result.push_back(1);
//				}
//				else if(it->widthRate > 0 &&  it->width > 500)
//				{
//					result.push_back(1);
//				}
//				else if(it->widthRate < 0 && it->width < 70)
//				{
//					result.push_back(1);
//				}
//				else
...
//			{
//				int sum = 0;
//				for(auto vHistiter = HistRect.rbegin();vHistiter != HistRect.rbegin() + 3; ++vHistiter)
//				{
